...
--*/
{
	...
	if (EOS_SUCCESS(Status)) {
		...
		if (Zero == Thread->State) {
			...
			//
			// 在此添加代码将线程恢复为就绪状态
			ListRemoveEntry(&Thread->StateListEntry);	// 将线程从挂起线程队列移除，在list.c中定义（第18行）
			// Thread->State==Zero满足就绪条件
			PspReadyThread(Thread);	// 将线程恢复为就绪状态，插入到就绪队列队尾，在sched.c中定义（第106行）
			PspThreadSchedule(); // 宏函数执行线程调度，有机会执行
			//
