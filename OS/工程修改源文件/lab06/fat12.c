...
--*/
{
	...
	//如果偏移量+要写的字节数超过了一个扇区的大小，说明要跨越这个要写的扇区，写入后面的一个或多个扇区中
	if(OffsetInSector + BytesToWrite > Vcb->Bpb.BytesPerSector){
		//先计算第一个扇区要写多少个字节，扇区大小减偏移量
		...
		if (!EOS_SUCCESS(Status))
			return Status;
		WriteCount += BytesToWriteFirstTime;
		//数据去掉开始的部分和最后剩余的部分，中间的部分一定是扇区大小的整数倍，计算出这个倍数，就是要写几次中间的扇区
		INT sharpTimes = (BytesToWrite - BytesToWriteFirstTime - BytesToWriteLastTime) / Vcb->Bpb.BytesPerSector;
		INT j = 0;
		for (j = 0; j<sharpTimes; ++j) {
			//顺着簇链向后查找下一个簇的簇号，如果没有就先分配
			PrevClusterNum = CurrentClusterNum;
			CurrentClusterNum = FatGetFatEntryValue(Vcb, PrevClusterNum);
			// 如果写入的起始位置还没有对应的簇，就增加簇
			if (0 == CurrentClusterNum || CurrentClusterNum >= 0xFF8) {
			...
			FirstSectorOfCluster = Vcb->FirstDataSector + (CurrentClusterNum - 2) * Vcb->Bpb.SectorsPerCluster;
			//写入当前扇区，因为写入完整的一个扇区的数据，所以偏移量为 0，写入字节数为一个扇区的大小
			//写入数据缓冲区(PCHAR)Buffer的指针要移动到对应的位置，地址！即起始位置+已经写完的字节数
			Status = IopReadWriteSector(Vcb->DiskDevice,
					FirstSectorOfCluster,
					0,
			...
			if (!EOS_SUCCESS(Status))
				return Status;
			WriteCount += Vcb->Bpb.BytesPerSector;
		}
